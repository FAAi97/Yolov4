...
from utils.utils import rotated_box_wh_iou_polygon, rotated_box_11_iou_polygon, to_cpu
...
class YoloLayer(nn.Module):
    ...
    def build_targets(self, pred_boxes, pred_cls, target, anchors, ignore_thres):
        ...
        if n_target_boxes > 0:  # Make sure that there is at least 1 box
            target_boxes = torch.cat((target[:, 2:self.num_b_b_attr-1] * nG, target[:, self.num_b_b_attr-1:8]), dim=-1)  # scale up x, y, w, h

            gxy = target_boxes[:, :2]
            gwh = target_boxes[:, 2:self.num_b_b_attr-3]
            gimre = target_boxes[:, self.num_b_b_attr-3:self.num_b_b_attr-1]

            # Get anchors with best iou
            ious_a_tg = torch.stack([rotated_box_wh_iou_polygon(anchor, gwh, gimre) for anchor in anchors])    
    ...
    def forward(self, x, targets=None, img_size=608, use_giou_loss=False):
        ...
        output = torch.cat(
            (
                pred_boxes[..., :self.num_b_b_attr-3].view(num_samples, -1, self.num_b_b_attr-3) * self.stride,
