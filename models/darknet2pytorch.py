    ...
    for block in blocks:
        if block["type"] == "net":
            ...
            if stride == 1 and kernel_size % 2:
                ...
                model = nn.MaxPool2d(kernel_size=kernel_size, stride=stride, padding=kernel_size // 2)
            elif stride == kernel_size:
                # You can use Maxpooldark instead, here is convenient to convert onnx.
                # Example: [maxpool] size=2 stride=2
                model = nn.MaxPool2d(kernel_size=kernel_size, stride=stride, padding=0)
            else:
                model = MaxPoolDark(kernel_size, stride)
            out_filters.append(prev_filters)
            prev_stride = stride * prev_stride
            out_strides.append(prev_stride)
            module_list.append(model)
            ...
            out_strides.append(prev_stride)
            module_list.append(Reorg(stride))
            ...
            module_list.append(Upsample_expand(stride))
            # module_list.append(Upsample_interpolate(stride))
            ...
            out_filters.append(prev_filters)
            out_strides.append(prev_stride)
            module_list.append(EmptyModule())
