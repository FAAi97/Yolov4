...
from data_process import kitti_aug_utils, kitti_bev_utils, kitti_utils
...
class KittiDataset(Dataset):
    def __init__(self, dataset_dir, mode='train', lidar_transforms=None, aug_transforms=None, multiscale=False,
        ...
        total_list = [x.strip() for x in open(split_txt_path).readlines()]
        if len(total_list) > 2400:
            self.image_idx_list = random.sample(total_list, 2400)
        else :
            self.image_idx_list = total_list
    ...
    def remove_invalid_idx(self, image_idx_list):
        ...
        for sample_id in image_idx_list:
            ...
            labels, noObjectLabels = kitti_bev_utils.read_labels_for_bevbox(objects)
            if not noObjectLabels:
                labels[:, 1:] = kitti_aug_utils.camera_to_lidar_box(labels[:, 1:], calib.V2C, calib.R0,
