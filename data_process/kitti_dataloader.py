...
# from kitti_aug_utils import Compose, OneOf, Random_Rotation, Random_Scaling, Horizontal_Flip, Cutout
...
if __name__ == '__main__':
    ...
    import data_process.kitti_utils
    # import kitti_bev_utils as bev_utils
    # import kitti_utils
    from utils.mayavi_viewer import show_image_with_boxes, merge_rgb_to_bev, invert_target
    ...
    parser.add_argument('--hflip_prob', type=float, default=0., help='The probability of horizontal flip')
    ...
    parser.add_argument('--show-train-data', action='store_true', help='If true, random padding if using mosaic augmentation')
    parser.add_argument('--output-width', type=int, default=608, help='the width of showing output, the height maybe vary')
    parser.add_argument('--save_img', action='store_true', help='If true, save the images')
    ...
    for batch_i, (img_files, imgs, targets) in enumerate(dataloader):
        if not (configs.mosaic and configs.show_train_data):
            ...
            calib = kitti_utils.Calibration(img_file.replace(".png", ".txt").replace("image_2", "calib"))
