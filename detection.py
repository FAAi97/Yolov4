...
from data_process import kitti_utils, kitti_bev_utils,distfile
...
from utils.mayavi_viewer import  Center_Point, distance_calculate, show_image_with_boxes, merge_rgb_to_bev, predictions_to_kitti_format
...
def main():
    ...
    for batch_idx, (img_paths, imgs_bev) in enumerate(test_dataloader):
        ...
        for detections in img_detections:
            ...
            confidences=[]
            pnt=[]
            for x, y, w, l, im, re, object_conf,*_, cls_pred in detections:
                if cls_pred != 0 :
                    yaw = np.arctan2(im, re)
                    confidences.append(object_conf)
                    # Draw rotated box
        ...
        img_rgb = show_image_with_boxes(img_rgb,confidences,objects_pred, calib, False)
        ...
        if configs.show_image:
            cv2.imshow("test-img1", out_img)
            cv2.moveWindow("test-img1",700,5)
